...
def init_db():
    ...
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS viaggio (
            ...
            contributo REAL NOT NULL,
            soste TEXT,
            bagaglio INTEGER DEFAULT 1,
            animali INTEGER DEFAULT 0,
            posti_max INTEGER NOT NULL,
            marca_auto TEXT NOT NULL,
            modello_auto TEXT NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS feedback (
            ...
            giudizio TEXT,
            id_viaggio INTEGER NOT NULL,
            id_autore_passeggero INTEGER NOT NULL,
...
# ── AUTISTI ────────────────────────────────────────────────
@app.route("/autisti/index.php", methods=["GET","POST","PUT","OPTIONS"])
def autisti():
    ...
    if request.method == "GET":
        ...
        if id_a:
            ...
            """, (id_a,)).fetchone()
            if not row:
                return jsonify({"errore": "Autista non trovato"}), 404
    ...
    if request.method == "POST":
        ...
        try:
            ...
            """, (b["nome"],b["cognome"],b["num_patente"],b["scadenza_patente"],
                  b["telefono"],b["email"],b.get("foto")))
            conn.commit()
            return jsonify({"successo": True, "id": conn.execute("SELECT last_insert_rowid()").fetchone()[0]}), 201
...
def passeggeri():
    ...
    if request.method == "GET":
        ...
        """, (id_p,)).fetchone()
        if not row:
            return jsonify({"errore": "Passeggero non trovato"}), 404
        return jsonify({"successo": True, "dati": dict(row)})
    ...
    if request.method == "POST":
        ...
        try:
            conn.execute("""
                ...
                VALUES (?,?,?,?,?)
            """, (b["nome"],b["cognome"],b["documento_identita"],b["telefono"],b["email"]))
            conn.commit()
            return jsonify({"successo": True, "id": conn.execute("SELECT last_insert_rowid()").fetchone()[0]}), 201
        except Exception as e:
            return jsonify({"errore": str(e)}), 409
...
def viaggi():
    ...
    if request.method == "GET":
        ...
        if id_v:
            row = conn.execute("""
                SELECT v.*, a.nome, a.cognome, a.telefono, a.email, a.foto,
                       ...
                       (v.posti_max - (
                           ...
                           WHERE id_viaggio=v.id_viaggio AND stato='accettata'
            ...
            """, (id_v,)).fetchone()
            if not row:
                return jsonify({"errore": "Viaggio non trovato"}), 404
            return jsonify({"successo": True, "dati": dict(row)})
    ...
    if request.method == "POST":
        ...
        try:
            conn.execute("""
                INSERT INTO viaggio
                    (citta_partenza,citta_arrivo,data_ora_partenza,tempo_stimato,
                     contributo,soste,bagaglio,animali,posti_max,
            ...
            conn.commit()
            return jsonify({"successo": True, "id_viaggio": conn.execute("SELECT last_insert_rowid()").fetchone()[0]}), 201
        except Exception as e:
...
def prenotazioni():
    ...
    if request.method == "GET":
        ...
        if id_pr:
            row = conn.execute("""
                SELECT pr.*,
                       pa.nome as nome_pass, pa.cognome as cog_pass, pa.email as email_pass,
                       v.citta_partenza, v.citta_arrivo, v.data_ora_partenza,
                       v.contributo, v.soste,
                ...
                JOIN autista a ON v.id_autista = a.id_autista
                WHERE pr.id_prenotazione=? AND pr.stato='accettata'
            """, (id_pr,)).fetchone()
            if not row:
        ...
        if id_v:
            ...
            params = [id_v]
            if voto_min:
                sql += " HAVING voto_medio >= ? OR voto_medio IS NULL"
                params.append(voto_min)
            rows = conn.execute(sql, params).fetchall()
            return jsonify({"successo": True, "dati": [dict(r) for r in rows]})
    ...
    if request.method == "POST":
        ...
        chk = conn.execute("""
            SELECT posti_max,
                   (SELECT COUNT(*) FROM prenotazione
                    WHERE id_viaggio=? AND stato='accettata') as occupati
            FROM viaggio WHERE id_viaggio=?
        """, (b["id_viaggio"], b["id_viaggio"])).fetchone()
        if not chk or (chk["posti_max"] - chk["occupati"]) <= 0:
