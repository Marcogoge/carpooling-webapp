 import { Component, OnInit } from '@angular/core';
  ...
  export class FeedbackFormComponent implements OnInit {
    ...
    tipo = ''; idViaggio = 0; idDestination = 0;
    utente: any;
    ...
    invia() {
      ...
      const payload: any = {
        ...
        tipo_feedback: this.tipo,
      };
 
      // FIX: distingui autore autista vs passeggero
      if (this.utente.tipo === 'passeggero') {
        payload.id_autore_passeggero = this.utente.id;
      } else {
        payload.id_autore_autista = this.utente.id;
      }
 
      if (this.tipo === 'per_autista') {
        payload.id_destinatario_autista = this.idDestination;
      } else {
        payload.id_destinatario_passeggero = this.idDestination;
      }
 
      this.fs.invia(payload).subscribe({
        next: () => {
          this.caricamento = false;
          this.messaggio = 'Recensione inviata!';
          setTimeout(() => {
