  ...
  export class CercaViaggiComponent {
    ...
    cerca() {
      ...
      let ds='';
      if(this.dataObj){const d=this.dataObj; ds=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;}
      this.vs.getViaggi(this.partenza,this.arrivo,ds).subscribe({
        next:(r:any)=>{ this.viaggi=r.dati; this.cercato=true; this.caricamento=false; },
