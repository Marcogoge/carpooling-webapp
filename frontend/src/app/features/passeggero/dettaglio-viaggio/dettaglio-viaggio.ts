  ...
  export class DettaglioViaggioComponent implements OnInit {
    ...
    ngOnInit(){
      ...
      this.vs.getViaggio(id).subscribe({
        next:(r:any)=>{ this.viaggio=r.dati; this.caricamento=false; },
        error:()=>{ this.caricamento=false; this.errore='Viaggio non trovato.'; }
      });
      this.fs.getFeedbackAutista(id).subscribe({ next:(r:any)=>this.feedback=r.dati });
    }
